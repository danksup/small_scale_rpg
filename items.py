...
def load_schema_from_reference(category):
    try:
    ...
    reference_key = next(iter(data))
    return list(data[reference_key].keys())


def push():
    category = input("category: ").strip()

    items_attr[category] = load_schema_from_reference(category)
...
def update():
    ...
    if not items_attr[category]:
        print("no reference schema found")
        return
